                    ...
                    <tr class="day" v-for="(week, index) in calendar" :key="index">
                        <td v-for="date in week" :key="date" :class="{ disabled: !date }">
                            <div class="date-container">
                                <span>{{ date || '' }}</span>
                                <img v-if="date && hasExpense(date)" src="./sticker.png" alt="달성" class="sticker" />
                            </div>
                        </td>
                    </tr>
                </tbody>
...
export default {
    ...
    methods: {
        async fetchExpenditureData() {
            ...
            try {
                ...
                if (response.status === 200) {
                    this.expenditureData.todoList = response.data;
                } else {
                    alert('데이터 조회 실패');
                }
            } catch (error) {
        ...
        hasExpense(date) {
            const formattedDate = `${this.selectedYear}-${String(this.selectedMonth).padStart(2, '0')}-${String(date).padStart(2, '0')}`;
            const expenseOnDate = this.expenditureData.todoList.find((item) => item.date === formattedDate && item.type === 'expense');
            const incomeOnDate = this.expenditureData.todoList.find((item) => item.date === formattedDate && item.type === 'income');
